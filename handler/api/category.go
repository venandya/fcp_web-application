...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	var reqBody = model.Category{}

	if err = c.BindJSON(&reqBody); err != nil {
		c.JSON(http.StatusInternalServerError, model.NewErrorResponse("Something went wrong while updating task!"))
		return
	}
	if err = ct.categoryService.Update(id, reqBody); err != nil {
		c.JSON(http.StatusInternalServerError, model.NewErrorResponse("Something went wrong while updating task!"))
		return
	}
	c.JSON(http.StatusOK, model.NewSuccessResponse("category update success")) // TODO: answer here
}
...
func (ct *categoryAPI) DeleteCategory(c *gin.Context) {
	...
	c.JSON(http.StatusOK, model.NewSuccessResponse("category delete success")) // TODO: answer here
}
...
func (ct *categoryAPI) GetCategoryList(c *gin.Context) {
	lists, err := ct.categoryService.GetList()
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
